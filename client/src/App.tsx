...
import AuditModeTabs from './components/AuditModeTabs'
import SupplierAuditPage from './pages/SupplierAuditPage'
...
export default function App() {
  ...
  const summary = useMemo(() => {
    ...
    return all
      ...
      .filter(x => x.a && (x.a.mv || x.a.pe) && ((x.a.note?.trim() || x.a.evidence?.trim())?.length))
      .map(x => ({
        id: x.q.id,
        clause: x.q.clause || '',
  ...
  async function importSchemaXLSX(file: File) {
    ...
    try {
      ...
      const pick = (row: any, keys: string[]): string => {
        const val = keys.map(k => row[k]).find(v => v !== undefined && v !== null && String(v).trim() !== '')
        return val !== undefined ? String(val).trim() : ''
      }

      for (const row of rows) {
        const processName = pick(row, ['Protsess','Protsess (küsimuse alla)','Department','Osakond','Process','Process Name'])
        const questionId  = pick(row, ['Küsimuse ID','Küsimus ID','Question ID','ID'])
        const questionText= pick(row, ['Küsimuse tekst','Küsimus','Question','Text'])
        const clause      = pick(row, ['Standardi nõue','Klausel','Clause'])
        const standardsRaw= pick(row, ['Standard','Standardid','Standards'])
        const guidance    = pick(row, ['Juhend auditeerijale','Juhend','Guidance'])
        ...
        if (questionId && questionText) {
          const stdsArr = standardsRaw ? standardsRaw.split(/[ ,;]+/).filter(Boolean) as Std[] : ([] as Std[])
          const q: Question = { id: questionId, text: questionText, clause: clause || undefined, stds: stdsArr, guidance: guidance || undefined }
  ...
  <div className="ml-auto flex items-center gap-2 no-print">
    {!token ? (
      ...
      {mode === 'tarnijaaudit' ? (
        <div className="mt-4">
          <SupplierAuditPage token={token} role={role} />
        </div>
      ) : (
        <>
          <div className="mt-4" />
