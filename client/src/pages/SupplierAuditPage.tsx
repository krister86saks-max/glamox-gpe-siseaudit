  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-6 gap-2">
        ...
        <input className="border p-2 rounded" placeholder="Auditeeritav"
          value={audit.auditee ?? ''} onChange={e=>setAudit({ ...audit, auditee: e.target.value })} />
        ...
        {audit.points.map(point => (
          <div key={point.id} className="border rounded-2xl p-4 shadow-sm">
            <div className="flex gap-2 items-center mb-2">
              <div className="px-3 py-2 border rounded bg-gray-50 w-14 text-center select-none">{/* index feel */}
                {audit.points.indexOf(point)+1}
              </div>
              <input
...
function MultiOptionsEditor({
  ...
  const addOpt = () => canEdit && onChange({ options: [...opts, { id: nanoid(), label: 'Uus valik' }] });

  const setLabel = (id: string, label: string) =>
    canEdit && onChange({ options: (sub.options ?? []).map(o => o.id === id ? { ...o, label } : o) });
  ...
  const remove = (id: string) =>
    canEdit && onChange({ options: (sub.options ?? []).filter(o => o.id !== id) });
