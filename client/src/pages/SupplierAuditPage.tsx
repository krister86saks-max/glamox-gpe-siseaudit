...
interface Props {
  token: string | null; // App.tsx-st
  role: Role;           // App.tsx-st
}
...
export default function SupplierAuditPage({ token, role }: Props) {
  ...
  // --- util: sügav koopia mallist auditisse ---
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
        ...
        <button
          className="border p-2 rounded bg-green-100 text-green-800 hover:bg-green-200"
          disabled={!tplId}
          onClick={() => {
        ...
        {audit.points.map((point) => (
          <div key={point.id} className="border rounded-2xl p-4 shadow-sm">
            <div className="flex gap-2 items-center mb-2">
              ...
              />
              <input
                className="border p-2 rounded flex-1 font-semibold text-gray-800"
...
function MultiOptionsEditor({
  ...
  const setLabel = (id: string, label: string) =>
    onChange({ options: (sub.options ?? []).map((o) => (o.id === id ? { ...o, label } : o)) });
