        ...
        {audit.points.map(point => (
          <div key={point.id} className="border rounded-2xl p-4 shadow-sm">
            <div className="flex gap-2 items-center mb-2">
              ...
              <input className="border p-2 rounded flex-1 font-semibold" placeholder="Punkti pealkiri"
                     value={point.title} onChange={e=>updatePoint(point.id, { title: e.target.value })} />
              <div className="ml-auto flex gap-2">
                {role === 'admin' && (
                  <>
                    <button className="px-3 py-1 border rounded no-print" onClick={() => addSub(point, 'open')}>+ Küsimus</button>
                    <button className="px-3 py-1 border rounded no-print" onClick={() => addSub(point, 'multi')}>+ Valikvastustega</button>
                    <button className="px-3 py-1 border rounded no-print" onClick={() => removePoint(point.id)}>Kustuta punkt</button>
                  </>
                )}
              </div>
            </div>
              ...
              {point.subQuestions.map(sub => (
                <div key={sub.id} className="border rounded-xl p-3">
                  <div className="flex gap-2 items-center">
                    ...
                    {role === 'admin' && (
                      <button className="px-2 py-1 border rounded no-print" onClick={() => removeSub(point, sub.id)}>Kustuta</button>
                    )}
                  </div>
                ...
                <div className="mt-2 grid md:grid-cols-2 gap-2">
                  {images[point.id]!.map((src, i) => (
                    <div key={i} className="border rounded p-1">
                      ...
                      <button className="text-xs underline mt-1 no-print" onClick={() => removeImage(point.id, i)}>Eemalda</button>
                    </div>
                  ))}
                </div>
              )}
    ...
    <div className="mt-2 space-y-2">
      {(sub.options ?? []).map(o => (
        <div key={o.id} className="flex items-center gap-2">
          ...
          {!readonly && <button className="px-2 py-1 border rounded no-print" onClick={() => remove(o.id)}>×</button>}
        </div>
      ))}
      {!readonly && <button className="px-2 py-1 border rounded no-print" onClick={addOpt}>+ Lisa valik</button>}
    </div>
  )
}
